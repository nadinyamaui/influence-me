...
<div class="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-6">
    <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
            ...
            <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-300">Draft a proposal with campaign planning and scheduled content.</p>
        </div>
    ...
    <form wire:submit="save" class="space-y-5 rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:field>
            ...
            <flux:textarea wire:model="content" rows="12" required />
        ...
        <section class="space-y-4">
            ...
            @foreach ($campaigns as $campaignIndex => $campaign)
                <article wire:key="create-campaign-{{ $campaignIndex }}" class="space-y-4 rounded-xl border border-zinc-200 p-4 dark:border-zinc-700">
                    <div class="grid gap-4 md:grid-cols-2">
                        <flux:field>
                            <flux:label>Existing Campaign (Optional)</flux:label>
                            <flux:select wire:model="campaigns.{{ $campaignIndex }}.id">
                        ...
                        <flux:field>
                            ...
                            <flux:input wire:model="campaigns.{{ $campaignIndex }}.name" />
                    ...
                    <flux:field>
                        ...
                        <flux:textarea wire:model="campaigns.{{ $campaignIndex }}.description" rows="3" />
                    ...
                    <div class="space-y-3">
                        ...
                        @foreach ($campaign['scheduled_items'] as $scheduledItemIndex => $scheduledItem)
                            <div wire:key="create-campaign-{{ $campaignIndex }}-item-{{ $scheduledItemIndex }}" class="grid gap-4 rounded-lg border border-zinc-200 p-4 md:grid-cols-2 dark:border-zinc-700">
                                <flux:field>
                                    <flux:label>Title</flux:label>
                                    <flux:input wire:model="campaigns.{{ $campaignIndex }}.scheduled_items.{{ $scheduledItemIndex }}.title" required />
                                    <flux:error name="campaigns.{{ $campaignIndex }}.scheduled_items.{{ $scheduledItemIndex }}.title" />
                                </flux:field>

                                <flux:field>
                                    <flux:label>Content Type</flux:label>
                                    <flux:select wire:model="campaigns.{{ $campaignIndex }}.scheduled_items.{{ $scheduledItemIndex }}.media_type" required>
                                ...
                                <flux:field>
                                    ...
                                    <flux:select wire:model="campaigns.{{ $campaignIndex }}.scheduled_items.{{ $scheduledItemIndex }}.instagram_account_id" required>
                                ...
                                <flux:field>
                                    ...
                                    <flux:input type="datetime-local" wire:model="campaigns.{{ $campaignIndex }}.scheduled_items.{{ $scheduledItemIndex }}.scheduled_at" required />
                                ...
                                <div class="md:col-span-2">
                                    <flux:field>
                                        ...
                                        <flux:textarea wire:model="campaigns.{{ $campaignIndex }}.scheduled_items.{{ $scheduledItemIndex }}.description" rows="3" />
