...
<div class="flex h-full w-full flex-1 flex-col gap-6">
    <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
            ...
            <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-300">Manage your client proposals and track campaign planning.</p>
        </div>

        <flux:button href="#" variant="primary" wire:navigate title="New Proposal" aria-label="New Proposal">
                        ...
                        @foreach ($proposals as $proposal)
                            <tr wire:key="proposal-row-{{ $proposal->id }}">
                                ...
                                <td class="px-4 py-3 text-zinc-700 dark:text-zinc-200">
                                    {{ number_format($proposal->campaigns->sum(fn($campaign) => $campaign->scheduledPosts->count())) }}
                                </td>
                                <td class="px-4 py-3">
                                    @if ($proposal->status === ProposalStatus::Draft)
                                        <span class="inline-flex rounded-full bg-zinc-100 px-2.5 py-1 text-xs font-medium text-zinc-700 dark:bg-zinc-800 dark:text-zinc-200">Draft</span>
