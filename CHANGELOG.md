...
- RFC 027: added scheduler automation in `routes/console.php` for full Instagram sync every 6 hours (`sync-all-instagram`, excluding accounts already `syncing`), profile/insights refresh hourly (`refresh-instagram-insights`), and token refresh daily for non-expired accounts expiring within 7 days (`refresh-instagram-tokens`).
