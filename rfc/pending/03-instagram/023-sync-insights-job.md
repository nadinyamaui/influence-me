...
Create a queued job that fetches insights (reach, impressions, saved, shares) for each media item from the Instagram Graph API and updates the `InstagramMedia` records.
...
3. Update the media record with: `reach`, `impressions`, `saved_count`, `shares_count`
4. Calculate `engagement_rate`:
   ```
   engagement_rate = ((likes + comments + saved + shares) / reach) * 100
   ```
   If reach is 0, set engagement_rate to 0
5. Rate limit awareness: check `canMakeRequest()` before each call, sleep if approaching limit
